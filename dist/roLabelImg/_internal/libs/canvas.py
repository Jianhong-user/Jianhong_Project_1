    ...
    def mouseReleaseEvent(self, ev):  
        ...
        if ev.button() == Qt.RightButton:
            ...
            if self.isPanning:
                self.isPanning = False
                self.restoreCursor()
            # 如果选中了顶点且形状可旋转，则不显示菜单（已经执行了旋转操作）
            elif self.selectedVertex() and self.selectedShape and self.selectedShape.isRotated:
                pass
            # 如果不是拖动操作，也不是旋转操作，则显示菜单
            elif not self.selectedVertex():
